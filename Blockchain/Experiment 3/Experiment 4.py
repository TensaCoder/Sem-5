...
def createPublicPrivateKeys(numOfUsers):
    ...
    for i in range(numOfUsers):
        ...
        # generate public key
        vk = sk.verifying_key
        # the keys are stored in the form of object
...
def findHash(blockNumber, blockData, previousHash):
    ...
    while hashValue[:4] != "0000":
        nonce += 1
        hashValue = pow.hash(str(blockNumber).encode() + str(nonce).encode() + str(blockData).encode() + str(previousHash).encode()).hexdigest()

    return hashValue, nonce


def createBlock(previousBlockNumber, previousHash, blockData):
...
def keyPairsConvert(keyPairs):
    for keys in keyPairs:
        keys[0] = keys[0].to_string().hex()
        keys[1] = keys[1].to_string().hex()
    return keyPairs
...
def main():
    ...
    keyPairs = keyPairsConvert(keyPairs)
